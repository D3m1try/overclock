/*
*  C Implementation: ics94215
*
* Description: ics94215
*
*
...
{
	int i, file, res;
	unsigned char buf[] = {0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x06, 0x20, 0x08,
	                       0x00, 0x10, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0xFF, 0xEA,
	                       0xAA, 0xFF, 0xFF};
	...
	for(i=0; pll[i].fsb; i++)
		if(pll[i].fsb == fsb)
		{
			...
			buf[CONTROLBYTE1] = ((buf[CONTROLBYTE] & 0x10)>>4) + ((buf[CONTROLBYTE] & 0x20)>>4) + \
			                    ((buf[CONTROLBYTE] & 0x30)>>4) + ((buf[CONTROLBYTE] & 0x40)>>4) + (1<<4);
		}
